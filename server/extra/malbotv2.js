...
function commands(DB, bid, cid, uid, rawtext){
	...
	let maltest = new RegExp(/\$maltest[^.](.+)/gi).exec(rawtext);
	...
	*/
	...
	if (maltest != null){
		check(DB, maltest[1], function(result){});
	}
...
function similarAnimes(DB, bid, criteria, callback){
	...
	malScraper.getResultsFromSearch(criteria, "character")
		.then((data) => {
			console.log(data[0].url);
			malScraper.getInfoFromCharacterURL(data[0].url).then((result)=> {
				
				let mparse = result.scrape.split("\t").join("").split("\n");
				let out = [];
				
				for (let i=0; i < mparse.length; i++){
					if (mparse[i].trim() !== ""){
						out.push(mparse[i].trim());
					}
				}
				
				//comparar linea con equivalencia.
				for (let l=0; l < out.length; l++){
					if (out[l].search("Source:") != -1 || out[l].search("Voice Actors") != -1){
						let resultado = [];
						let pre2 = out[l-2];
						let pre3 = out[l-3];						
						
						if (pre2.search(".") !== -1 && pre2.search(":") === -1){
							if (pre3.search(".") !== -1 && pre3.search(":") === -1){
								console.log(pre3);
								resultado.push(pre3);
							} 
							resultado.push(pre2);
						}
						resultado.push(out[l-1]);					
...
function createBox(DB, data, callback){
	...
	//obtener body de la página en vez del rss
	axios.get(data.link).then(function(response){
		const $ = cheerio.load(response.data);
		
		//parse del contenido de la publicacion.
		let body = $("div.content").text().trim();
		mbox.content.body = cparser.htmlParser(body) + '</br><span style="color: #5fb732">URL: </span>' + '<a target="_blank" href="' + data.guid + '">' + data.guid + '</a>';
